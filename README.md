...
The resulting dataset is a JSON file, found in [`data/arsonor_data.json`](data/arsonor_data.json), that contains information in this form:
...
It contains 58 records corresponding to the number of articles currently presents on the site.
But each article is quite a long text so each record need to pass a chunking step in order to be indexed properly.
